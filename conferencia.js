...
document.getElementById('extract-btn').onclick = function() {
    const htmlInput = document.getElementById('html-input').value;alet bancoDeDados = {
    pendentes: [],
    conferidos: [],
    foraRota: [],
    idRota: "",
...
document.getElementById('extract-btn').onclick = function() {
    ...
    if (encontrados.length > 0) {
        ...
        alert("Nenhum ID de 11 dígitos encontrado.");
    }
};
...
function atualizarPainel() {
    ...
    document.getElementById('conferidos-list').innerHTML = bancoDeDados.conferidos.map(p => `<li class="list-group-item list-group-item-success p-1">${p}</li>`).join('');
    document.getElementById('faltantes-list').innerHTML = bancoDeDados.pendentes.map(p => `<li class="list-group-item list-group-item-danger p-1">${p}</li>`).join('');
    document.getElementById('fora-rota-list').innerHTML = bancoDeDados.foraRota.map(p => `<li class="list-group-item list-group-item-warning p-1">${p}</li>`).join('');
}
